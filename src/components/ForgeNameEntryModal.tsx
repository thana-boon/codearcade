...
interface ForgeNameEntryModalProps {
  level: Difficulty;
  score: number;
  timeSeconds: number;
  runCount: number;
...
// modal กรอกชื่อสไตล์ตู้เกมอาเขต เมื่อทำคะแนน Function Forge ติด top 10
// โครงเดียวกับ NameEntryModal ของ Bug Hunt แต่ส่ง runCount และยิงไป API ของ Function Forge
export default function ForgeNameEntryModal({
  level,
  score,
  timeSeconds,
  runCount,
  ...
  const submit = async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/function-forge/leaderboard/submit", {
        method: "POST",
