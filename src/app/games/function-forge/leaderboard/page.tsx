...
import type { Difficulty, ForgeLeaderboardRow } from "@/lib/types";

export default function ForgeLeaderboardPage() {
  const [active, setActive] = useState<Difficulty>("noob");
  const [rows, setRows] = useState<ForgeLeaderboardRow[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/function-forge/leaderboard/${active}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <h1 className="mb-6 text-3xl font-bold">🏆 Leaderboard · Function Forge</h1>
