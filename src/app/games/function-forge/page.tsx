...
import { FORGE_META, FORGE_ORDER } from "@/lib/levels/function-forge-meta";
import { getForgeLevelsByDifficulty } from "@/lib/levels/function-forge-levels";

export default function FunctionForgeLevelSelect() {
  return (
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col px-6 py-12">
      ...
      <header className="mb-10">
        <h1 className="text-3xl font-bold sm:text-4xl">🔨 Function Forge</h1>
        <p className="mt-2 text-muted">
          เขียนฟังก์ชัน Python ให้ผ่าน test case ทุกข้อ (มีทั้ง test ที่เห็นและ
          test ที่ซ่อนไว้) เลือกระดับได้อิสระ ไม่มีการปลดล็อก
        </p>
      </header>

      <section className="grid gap-4">
        {FORGE_ORDER.map((level) => {
          const meta = FORGE_META[level];
          const count = getForgeLevelsByDifficulty(level).length;
          return (
            <Link
              key={level}
              href={`/games/function-forge/play?level=${level}`}
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-xs text-muted">แนวโจทย์: {meta.algoTypes}</p>
              </div>
