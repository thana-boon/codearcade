...
// POST /api/function-forge/leaderboard/check { level, score } → { qualifies: boolean }
...
export async function POST(req: Request) {
  try {
    ...
    const top = await db
      .select({ score: functionForgeLeaderboard.score })
      .from(functionForgeLeaderboard)
      .where(eq(functionForgeLeaderboard.level, level))
      .orderBy(desc(functionForgeLeaderboard.score))
      .limit(10);
